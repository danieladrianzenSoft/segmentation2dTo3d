...
def get_files(folder_path):
    ...
    print(f"Searching for .dat and .json files in: {folder_path}")

    all_files = list_files_by_extension(folder_path, [".dat", ".json"])
...
def validate_input(config, dat_files, json_files):
    file_type = config["file_type"]
    file_index = config["file_index"]

    if file_type not in ["dat", "json"]:
        print("Invalid file type. Please choose 'dat' or 'json'.")
        sys.exit(1)

    file_count = len(dat_files) if file_type == "dat" else len(json_files)
